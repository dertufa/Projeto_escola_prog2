...
class Professor():
	...
	def ver_nota_professor(self):
		...
		comando = f'SELECT nome_aluno , disciplina ,tipo_avaliacao, nota FROM avaliacoes ORDER BY name WHERE disciplina = "{self.disciplina_professor}" '
	...
	def inserir_nota(self):
		...
		cursor.execute(comando)
		nome_aluno_inserir_nota = cursor.fetchone()
		print(type(nome_aluno_inserir_nota))
		cursor.close()
		conexao.close()

		conexao = base.obter_conexao()
		cursor = conexao.cursor()
		comando = f'SELECT cpf_aluno FROM aluno WHERE id_aluno = "{p_seleciona_aluno}"'
		cursor.execute(comando)
		cpf_aluno_inserir_nota = cursor.fetchall()
		cursor.close()
		conexao.close()
